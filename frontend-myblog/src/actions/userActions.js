...
export const userDetailsWithPathAction = (path) => async (dispatch) => {
  try {
...
export const userDetailsSlugAction = (user) => async (dispatch) => {
  try {
    const slug = user.slug;
    dispatch({ type: typeAction.USER_DETAILS_REQUEST });
    const data = await (await axios.get("/api/v1/users/" + slug)).data;
    dispatch({ type: typeAction.USER_DETAILS_SUCCESS, payload: data });
  } catch (error) {
    dispatch({ type: typeAction.USER_DETAILS_FAIL, payload: error.message });
  }
