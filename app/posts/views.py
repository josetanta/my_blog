...
def post(slug):
	...
	if current_user == post.author:
		if form.validate_on_submit():
			...
			image_del = os.path.join(current_app.root_path,"html\\static\\uploads\\posts",post.upload)
...
def comment(slug):
	...
	if current_user.is_authenticated and c_form.validate_on_submit():
		post = Post.query.filter_by(slug = slug).first()
		comment = Comment(body = c_form.body.data, post = post, author = current_user._get_current_object())
